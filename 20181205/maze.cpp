...
int main(void){
    ...
    while(!coordinate.empty()){
        ...
        /* Left */
        if((maze[current.locx-1][current.locy] != '#') && (current.locx-1) >= 0){
            coordinate.push((location){current.locx-1, current.locy, current.dist+1});
        }

        /* Right */
        if((maze[current.locx+1][current.locy] != '#') && (current.locx+1) < rowL){
            coordinate.push((location){current.locx+1, current.locy, current.dist+1});
        }

        /* Up */
        if((maze[current.locx][current.locy-1] != '#') && (current.locy-1) >= 0){
            coordinate.push((location){current.locx, current.locy-1, current.dist+1});
        }

        /* Down */
        if((maze[current.locx][current.locy+1] != '#') && (current.locy+1) < colL){
            coordinate.push((location){current.locx, current.locy+1, current.dist+1});
        }
    }
